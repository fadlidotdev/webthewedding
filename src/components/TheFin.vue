<template>
  <footer
    class="relative flex flex-col items-center justify-center gap-6 px-5 py-8 pb-12 overflow-hidden section-fin text-light bg-primary-foreground"
  >
    <p class="text-sm text-center opacity-75" data-aos="fade-down" data-aos-offset="100">
      Atas kehadiran doa dan restu dari Bapak/Ibu/saudara/i sekalian kami ucapkan termakasih.
    </p>

    <h1
      class="text-4xl font-display"
      style="text-shadow: rgba(0, 0, 0, 0.1) 2px 2px 0px"
      data-aos="zoom-in"
      data-aos-offset="100"
    >
      Nanda &amp; Egi
    </h1>

    <p class="text-sm opacity-75" data-aos="fade-up" data-aos-offset="100">Sabtu, 27 April 2024</p>

    <img
      src="/ornaments/ornament-white-flower.png"
      class="w-16 -scale-x-[1] rotate-[120deg] -bottom-10 absolute"
      alt=""
    />
  </footer>
</template>
